...
for ($s = 0; $s -lt $simCount; $s++) {
    ...
    for ($turn = 1; $turn -le 20; $turn++) {
        ...
        $rCash = 0.0
        $rDBond = 0.04 + ((Get-Random -Minimum 0.0 -Maximum 1.0) * 2 - 1) * 0.03
        $rFBond = 0.05 + ((Get-Random -Minimum 0.0 -Maximum 1.0) * 2 - 1) * 0.08
        $rDStock = 0.08 + ((Get-Random -Minimum 0.0 -Maximum 1.0) * 2 - 1) * 0.18
        $rFStock = 0.03 + ((Get-Random -Minimum 0.0 -Maximum 1.0) * 2 - 1) * 0.40
